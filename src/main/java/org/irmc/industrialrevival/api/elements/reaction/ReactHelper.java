...
public class ReactHelper {
    ...
    public static ReactResult react(@NotNull Environment environment, @NotNull Set<ReactCondition> conditions, @NotNull List<ItemStack> items) {
        Map<ChemicalCompoundContainer, ItemStack> reactables = new HashMap<>();
        for (ItemStack item : items) {
            if (IndustrialRevivalItem.getByItem(item) instanceof ChemicalCompoundContainer cr) {
     ...
     * @return the getProduce of the reaction
     */
    public static @NotNull ReactResult react(@NotNull Environment environment, @NotNull Set<ReactCondition> conditions, @NotNull Map<ChemicalCompoundContainer, ItemStack> reactables) {
        Map<ChemicalCompound, Double> reactants = new HashMap<>();
        for (ChemicalCompoundContainer reactable : reactables.keySet()) {
    ...
    public static ReactResult react0(@NotNull Environment environment, @NotNull Set<ReactCondition> conditions, @NotNull Map<ChemicalCompound, Double> reactants) {
        List<ChemicalFormula> formulas = new ArrayList<>(IndustrialRevival.getInstance().getRegistry().getChemicalFormulas().values());
